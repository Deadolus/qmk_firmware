...
void dance_blue_finished(tap_dance_state_t *state, void *user_data) {
  if(get_mods()) return;
  if (state->count == 1) {
...
/* 0: ISO DE qwertz, SPACECADET and TAPDANCE */
  ...
  [_LVL1_] = LAYOUT_65_iso_blocker(
      ...
      _______, TO(_LVL2_), TO(_LVL3_), TO(_LVL4_), TO(_LVL5_), TO(_LVL6_), TO(_LVL7_), _______, _______, _______, _______, _______, _______, TO(0),   _______,
      _______, KC_SCRL,     KC_BRIU,    KC_BRID,    _______,       _______,      _______, _______, _______, _______, _______, _______, _______, KC_VOLU, _______,
...
const uint8_t PROGMEM ledmap[][4] = {
  ...
  [_LVL5_] = {HSV_MAGENTA,   LYR_SOLID},
  [_LVL6_] = {HSV_YELLOW,   LYR_SOLID},
  [_LVL7_] = {C_HSV_DARKGOLD,   LYR_SOLID}
};
...
//make sure we can enter numbers when entering blue layer -> numlock on
  if(IS_LAYER_ON_STATE(state, _LVL2_)) {
